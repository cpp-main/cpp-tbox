...
App::App(Context &ctx) : Module("echo_server", ctx), server_(new TcpServer(ctx.loop())) {}
...
bool App::onInit(const tbox::Json &cfg)
{
    ...
    if (!server_->initialize(SockAddr::FromString(js_bind.get<std::string>()), 2)) return false;

...
int main(int argc, char *argv[]) {
    ...
    LogInfo("second:%d, week_mask:%s, timezone:%d", seconds_from_00,
        week_mask.c_str(), timezone_offset_minutes);

    Loop* sp_loop = Loop::New();
    SetScopeExitAction([sp_loop] { delete sp_loop; });

    timer::WeeklyTimer tmr(sp_loop);
    tmr.initialize(seconds_from_00, week_mask, timezone_offset_minutes);

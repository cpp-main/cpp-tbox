...
If the module has already been built, you can run its test binary directly: `.build/<name>/test`. For example, to run the `http` module's unit tests, just execute `.build/http/test`.
...
The `tools/` directory contains auxiliary utilities (not part of the framework library):

- **tools/trace** — converts binary trace data generated by `modules/trace` into human-readable text and JSON files viewable as icicle diagrams in Chrome's `chrome://tracing` viewer.
- **tools/graphviz_render** — a Python GUI tool for real-time Graphviz rendering. It watches a named pipe (FIFO) and displays the Graphviz data on screen as it arrives.
- **tools/check_and_archive_log_files.sh** — log archiving helper script.

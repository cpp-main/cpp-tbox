...
| websocket | WebSocket库 | 在http的基础上实现了WebSocket的Server与Client模块，遵循RFC 6455与RFC 7692(permessage-deflate压缩)。支持：Text/Binary分类型回调(右值引用)、分片发送(可配置分片大小)、分片接收完整后统一解压再回调 |

#ifndef TBOX_THREAD_POOL_H_20171003
#define TBOX_THREAD_POOL_H_20171003
...
class ThreadPool {
  public:
    ...
    int execute(NonReturnFunc backend_task, NonReturnFunc main_cb, int prio);
  ...
  private:
    ...
    std::set<int> doing_tasks_set_;   //!< 记录正在从事的任务

    size_t idle_thread_num_ = 0;    //!< 空间线程个数
    std::map<int, std::thread*> threads_;    //!< 线程对象
...
#endif //TBOX_THREAD_POOL_H_20171003

...
StrIdRpc::StrIdRpc(event::Loop *loop)
    : request_timeout_(loop)
    , respond_timeout_(loop)
{
    using namespace std::placeholders;
    request_timeout_.setCallback(std::bind(&StrIdRpc::onRequestTimeout, this, _1));
    respond_timeout_.setCallback(std::bind(&StrIdRpc::onRespondTimeout, this, _1));

    id_gen_func_ = [] { return util::GenUUID(); };
}
...
bool StrIdRpc::initialize(Proto *proto, int timeout_sec)
{
    using namespace std::placeholders;
    ...
    proto->setRecvCallback(
        std::bind(&StrIdRpc::onRecvRequest, this, _1, _2, _3),
        std::bind(&StrIdRpc::onRecvRespond, this, _1, _2, _3)
    );
    proto_ = proto;
...
void StrIdRpc::request(const std::string &method, const Json &js_params, RequestCallback &&cb)
{
    RECORD_SCOPE();
    IdType id;
    if (cb && id_gen_func_) {
...
void StrIdRpc::request(const std::string &method, RequestCallback &&cb)
{
    request(method, Json(), std::move(cb));
}

void StrIdRpc::notify(const std::string &method, const Json &js_params)
{
    request(method, js_params, nullptr);
}

void StrIdRpc::notify(const std::string &method)
{
    request(method, Json(), nullptr);
}

void StrIdRpc::addService(const std::string &method, ServiceCallback &&cb)
{
    method_services_[method] = std::move(cb);
}

void StrIdRpc::respond(IdTypeConstRef id, int errcode, const Json &js_result)
{
    RECORD_SCOPE();
    if (id.empty()) {
        LogWarn("send id.empty() respond");
        return;
    }
...
void StrIdRpc::respond(IdTypeConstRef id, const Json &js_result)
{
    RECORD_SCOPE();
    if (id.empty()) {
        LogWarn("send id.empty() respond");
...
void StrIdRpc::respond(IdTypeConstRef id, int errcode)
{
    RECORD_SCOPE();
    if (id.empty()) {
        LogWarn("send id.empty() respond");
...
void StrIdRpc::onRecvRequest(IdTypeConstRef id, const std::string &method, const Json &js_params)
{
    RECORD_SCOPE();
...
void StrIdRpc::onRecvRespond(IdTypeConstRef id, int errcode, const Json &js_result)
{
    RECORD_SCOPE();
...
void StrIdRpc::onRequestTimeout(IdTypeConstRef id)
{
    auto iter = request_callback_.find(id);
    if (iter != request_callback_.end()) {
        if (iter->second)
...
void StrIdRpc::onRespondTimeout(IdTypeConstRef id)
{

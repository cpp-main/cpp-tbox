...
{
  public:
    Context(event::Loop *loop, DBusConnection *dbus_conn) : loop_(loop), dbus_conn_(dbus_conn)
    {
        ::dbus_connection_ref(dbus_conn_);
    }

    ~Context() { ::dbus_connection_unref(dbus_conn_); }
...
class WatchHandler
{
  public:
    WatchHandler(Context *ctx, DBusWatch *dbus_watch)
        : ctx_(ctx), dbus_watch_(dbus_watch), tbox_fd_(ctx->loop()->newFdEvent("dbus_watch"))
    {
  ...
  protected:
    void onWatch(short events)
    {
        ...
        if (events & tbox::event::FdEvent::kReadEvent) dbus_flags |= DBUS_WATCH_READABLE;
        if (events & tbox::event::FdEvent::kWriteEvent) dbus_flags |= DBUS_WATCH_WRITABLE;
        if (events & tbox::event::FdEvent::kExceptEvent) dbus_flags |= DBUS_WATCH_ERROR;
...
class TimeoutHandler
{
  public:
    TimeoutHandler(event::Loop *loop, DBusTimeout *dbus_timeout)
        : tbox_timer_(loop->newTimerEvent("dbus_timeout")), dbus_timeout_(dbus_timeout)
    {
        int interval_ms = ::dbus_timeout_get_interval(dbus_timeout);
        tbox_timer_->initialize(std::chrono::milliseconds(interval_ms),
...
void AttachLoop(DBusConnection *dbus_conn, event::Loop *loop)
{
    ...
    ::dbus_connection_set_watch_functions(
        dbus_conn, _AddWatch, _RemoveWatch, _ToggledWatch, watch_ctx, _ContextDeleter);

    //! 注册超时事件的创建与删除方法函数与数据
    auto timeout_ctx = new Context(loop, dbus_conn);
    ::dbus_connection_set_timeout_functions(
        dbus_conn, _AddTimeout, _RemoveTimeout, _ToggledTimeout, timeout_ctx, _ContextDeleter);

    //! 注册异步派发动作的方法函数与数据
    auto dispatch_ctx = new Context(loop, dbus_conn);
    ::dbus_connection_set_dispatch_status_function(
        dbus_conn, _DispatchStatus, dispatch_ctx, _ContextDeleter);

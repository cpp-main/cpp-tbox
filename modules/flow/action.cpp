...
Action::Action(event::Loop &loop, const std::string &type)
    : loop_(loop), id_(++_id_alloc_counter_), type_(type)
{}
...
void Action::toJson(Json &js) const
{
    js["id"] = id_;
    js["type"] = type_;
    if (!label_.empty()) js["label"] = label_;
...
bool Action::start()
{
    ...
    if (timer_ev_ != nullptr) timer_ev_->enable();
...
bool Action::pause()
{
    if (state_ == State::kPause) return true;
    ...
    if (timer_ev_ != nullptr) timer_ev_->disable();
...
bool Action::stop()
{
    if (state_ != State::kRunning && state_ != State::kPause) return true;
...
void Action::setTimeout(std::chrono::milliseconds ms)
{
    ...
    if (state_ == State::kRunning) timer_ev_->enable();
}
...
bool Action::finish(bool is_succ)
{
    if (state_ != State::kFinished) {
        LogDbg("action %d:%s[%s] finished, is_succ: %s",
               id_,
               type_.c_str(),
               label_.c_str(),
               (is_succ ? "succ" : "fail"));
...
std::string ToString(Action::State state)
{
    ...
    if (index < NUMBER_OF_ARRAY(tbl)) return tbl[index];
    return "unknown";
}

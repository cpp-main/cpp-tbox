...
ActionExecutor::~ActionExecutor()
{
    ...
    for (auto &action_deque : action_deque_array_) {
        for (auto item : action_deque) delete item.action;
        action_deque.clear();
    }
}
...
ActionExecutor::ActionId ActionExecutor::append(Action *action, int prio)
{
    ...
    Item item = {.id = allocActionId(), .action = action};
...
{
    if (curr_action_deque_index_ == -1) return -1;
...
{
    if (curr_action_deque_index_ == -1) return false;
...
bool ActionExecutor::cancel(ActionId action_id)
{
    ...
    for (auto &action_deque : action_deque_array_) {
        auto iter = std::find_if(action_deque.begin(),
                                 action_deque.end(),
...
ActionExecutor::ActionId ActionExecutor::allocActionId()
{
    return ++action_id_alloc_counter_;
}
...
void ActionExecutor::schedule()
{
    ...
    while (true) {
        ...
        if (ready_deque_index == -1) {
            ++cb_level_;
            if (all_finished_cb_) all_finished_cb_();
            --cb_level_;
            return;
        }

        //! 如果高优先级的队列比当前队列优先级还高，则要先暂停当前队列头部的动作
        if (curr_action_deque_index_ != -1 && ready_deque_index < curr_action_deque_index_) {
        ...
        while (!ready_deque.empty()) {
            ...
            if (item.action->state() == Action::State::kIdle) {
                if (item.action->start()) {
                    ...
                    ++cb_level_;
                    if (action_started_cb_) action_started_cb_(item.id);
                    --cb_level_;
                } else {
                    ...
                    ++cb_level_;
                    if (action_finished_cb_) action_finished_cb_(item.id);
                    --cb_level_;
                }

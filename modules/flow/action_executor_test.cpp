...
TEST(ActionExecutor, OneAction)
{
    ...
    ActionExecutor exec;
    exec.setActionStartedCallback([&](ActionExecutor::ActionId id) {
        EXPECT_EQ(id, 1);
        is_action_started_cb_done = true;
    });

    exec.setActionFinishedCallback([&](ActionExecutor::ActionId id) {
        EXPECT_EQ(id, 1);
    ...
    exec.append(new FunctionAction(*loop, [&] {
        is_run = true;
        return true;
    }));
...
TEST(ActionExecutor, Prio)
{
    ...
    exec.setAllFinishedCallback([loop] { loop->exitLoop(); });
    exec.setActionStartedCallback([&](ActionExecutor::ActionId id) { start_seq.push_back(id); });
    exec.setActionFinishedCallback([&](ActionExecutor::ActionId id) { finish_seq.push_back(id); });

    int index = 0;

    //! 加入延时动作1，防止立即开始执行
    exec.append(new SleepAction(*loop, std::chrono::milliseconds(10)), 1);

...
SequenceAction::SequenceAction(event::Loop &loop) : Action(loop, "Sequence") {}

SequenceAction::~SequenceAction()
{
    for (auto action : children_) delete action;
}
...
{
    if (index_ < children_.size()) return children_.at(index_)->stop();
    return false;
}

bool SequenceAction::onPause()
{
    if (index_ < children_.size()) return children_.at(index_)->pause();
    return false;
}

bool SequenceAction::onResume()
{
    if (index_ < children_.size()) return children_.at(index_)->resume();
    return false;
}
...
void SequenceAction::startOtheriseFinish(bool is_succ)
{
    if (index_ < children_.size()) {
        if (!children_.at(index_)->start()) finish(false);
    } else {
        finish(is_succ);
    }
}

...
LoopIfAction::LoopIfAction(event::Loop &loop, Action *if_action, Action *exec_action)
    : Action(loop, "LoopIf"), if_action_(if_action), exec_action_(exec_action)
{
    TBOX_ASSERT(if_action != nullptr);
    TBOX_ASSERT(exec_action != nullptr);

    if_action_->setFinishCallback([this](bool is_succ) {
        if (is_succ) {
            exec_action_->start();
        } else {
    ...
    exec_action_->setFinishCallback([this](bool) {
        if (state() == State::kRunning) {
            if_action_->reset();
            exec_action_->reset();

            if (!if_action_->start()) finish(finish_result_);
        }
    });
}

...
TEST(SleepAction, Basic)
{
    ...
    action.setFinishCallback([loop, &is_finished](bool is_succ) {
        loop->exitLoop();
        is_finished = true;
        (void)is_succ;
    });
...
TEST(SleepAction, Accuracy)
{
    ...
    action_50ms.setFinishCallback([&](bool is_succ) {
        ts_50ms = std::chrono::steady_clock::now();
        (void)is_succ;
    });
    action_100ms.setFinishCallback([&](bool is_succ) {
        ts_100ms = std::chrono::steady_clock::now();
        (void)is_succ;
    });
    action_200ms.setFinishCallback([&](bool is_succ) {
        ts_200ms = std::chrono::steady_clock::now();
        (void)is_succ;
    });
    ...
    auto cout_50 =
        std::chrono::duration_cast<std::chrono::milliseconds>(ts_50ms - ts_start).count();
    EXPECT_LE(cout_50, 51);
    EXPECT_GE(cout_50, 49);
    auto cout_100 =
        std::chrono::duration_cast<std::chrono::milliseconds>(ts_100ms - ts_start).count();
    EXPECT_LE(cout_100, 101);
    EXPECT_GE(cout_100, 99);
    auto cout_200 =
        std::chrono::duration_cast<std::chrono::milliseconds>(ts_200ms - ts_start).count();
...
TEST(SleepAction, GenLoopAction)
{
    ...
    body_action->append(new FunctionAction(*loop, [&] {
        ts.push_back(std::chrono::steady_clock::now());
        ++index;
        return true;
    }));
    ...
    for (size_t i = 0; i < NUMBER_OF_ARRAY(time_tbl); ++i) {
        ...
        int count =
            std::chrono::duration_cast<std::chrono::milliseconds>(ts[i + 1] - ts[i]).count();

...
RepeatAction::RepeatAction(event::Loop &loop, Action *child, size_t times, Mode mode)
    : Action(loop, "Repeat"), child_(child), repeat_times_(times), mode_(mode)
{
    TBOX_ASSERT(child != nullptr);

    child_->setFinishCallback([this](bool is_succ) {
        if ((mode_ == Mode::kBreakSucc && is_succ) || (mode_ == Mode::kBreakFail && !is_succ)) {
            finish(is_succ);

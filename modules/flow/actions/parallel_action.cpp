...
ParallelAction::ParallelAction(event::Loop &loop) : Action(loop, "Parallel") {}
...
bool ParallelAction::onStart()
{
    for (size_t index = 0; index < children_.size(); ++index) {
        ...
        if (!action->start()) fail_set_.insert(index);
    }
...
{
    for (Action *action : children_) action->stop();
    return true;
}
...
bool ParallelAction::onPause()
{
    for (Action *action : children_) {
        if (action->state() == Action::State::kRunning) action->pause();
    }
    return true;
}
...
{
    for (Action *action : children_) {
        if (action->state() == Action::State::kPause) action->resume();
    }
    return true;
}

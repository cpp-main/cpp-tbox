...
std::string ToString(const std::chrono::milliseconds msec)
{
    ...
    oss << days << ' ';
    oss << setfill('0');
    oss << setw(2) << hours << ':' << setw(2) << mins << ':' << setw(2) << secs << '.' << setw(3)
        << msecs;
...
bool ContextImp::initialize(const Json &cfg)
{
    ...
    if (!initThreadPool(js_thread_pool)) return false;
...
bool ContextImp::initThreadPool(const Json &js)
{
    ...
    if (!sp_thread_pool_->initialize(thread_pool_min, thread_pool_max)) return false;
...
bool ContextImp::initTelnetd(const Json &js)
{
    ...
    if (util::json::GetField(js, "bind", telnetd_bind)) {
        if (sp_telnetd_->initialize(telnetd_bind)) telnetd_init_ok = true;
    }
    return true;
}
...
bool ContextImp::initRpc(const Json &js)
{
    ...
    if (util::json::GetField(js, "bind", tcp_rpc_bind)) {
        if (sp_tcp_rpc_->initialize(tcp_rpc_bind)) tcp_rpc_init_ok = true;
    }
    return true;
}
...
{
    if (telnetd_init_ok) sp_telnetd_->start();

    if (tcp_rpc_init_ok) sp_tcp_rpc_->start();
...
{
    if (tcp_rpc_init_ok) sp_tcp_rpc_->stop();

    if (telnetd_init_ok) sp_telnetd_->stop();
}
...
void ContextImp::initShell()
{
...

    {
...

        {
            ...
            {
                auto func_node =
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (print_usage) oss << "Usage: " << args[0] << " <num>\r\n";
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().run_in_loop_delay =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
                            oss << sp_loop_->water_line().run_in_loop_delay.count() / 1000
                                << " us\r\n";
                        }
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().run_next_delay =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
                            oss << sp_loop_->water_line().run_next_delay.count() / 1000
                                << " us\r\n";
                        }
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().event_cb_cost =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().run_cb_cost =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().wake_delay =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
                    ...
                    wp_nodes->createFuncNode([this](const Session &s, const Args &args) {
                        ...
                        if (args.size() >= 2) {
                            ...
                            if (CatchThrowQuietly(may_throw_func)) {
                                ...
                                sp_loop_->water_line().timer_delay =
                                    std::chrono::microseconds(value);
                                oss << "done\r\n";
                            }
                        } else {
            ...
            auto func_node = wp_nodes->createFuncNode(
                [this](const Session &s, const Args &args) {
                    ...
                    auto ts = std::chrono::duration_cast<std::chrono::milliseconds>(
                                  start_time_point().time_since_epoch())
                                  .count();
                    {
                        time_t sec = ts / 1000;
                        struct tm tm;

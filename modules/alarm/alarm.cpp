...
Alarm::Alarm(event::Loop *wp_loop)
    : wp_loop_(wp_loop), sp_timer_ev_(wp_loop->newTimerEvent("Alarm::sp_timer_ev_"))
{
...
bool Alarm::enable()
{
    if (state_ == State::kInited) {
        if (onEnable()) return activeTimer();
    }
...
void Alarm::cleanup()
{
    if (state_ < State::kInited) return;
...
uint32_t Alarm::remainSeconds() const
{
    if (state_ == State::kRunning) {
        ...
        if (ret == 0) return target_utc_ts_ - utc_tv.tv_sec;
    }
    return 0;
}
...
bool Alarm::activeTimer()
{
    ...
    int timezone_offset_seconds =
        using_independ_timezone_ ? timezone_offset_seconds_ : GetSystemTimezoneOffsetSeconds();
    //! 计算需要等待的秒数
    ...
    //! 启动定时器
    sp_timer_ev_->initialize(std::chrono::milliseconds(wait_milliseconds),

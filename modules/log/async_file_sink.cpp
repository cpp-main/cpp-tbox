...
void AsyncFileSink::setFilePath(const std::string &file_path)
{
    ...
    if (file_path_.length() > 1 && file_path_.at(file_path_.length() - 1) == '/')
        file_path_.pop_back();
...
{
    if (pid_ == 0 || !checkAndCreateLogFile()) return;
    ...
    if (total_write_size_ >= file_max_size_) CHECK_CLOSE_RESET_FD(fd_);
}

bool AsyncFileSink::checkAndCreateLogFile()
{
    if (fd_ >= 0) {
        if (util::fs::IsFileExist(log_filename_)) return true;
        CHECK_CLOSE_RESET_FD(fd_);
    }

    //!检查并创建路径
    if (!util::fs::MakeDirectory(file_path_, false)) {
        cerr << "Err: create director " << file_path_ << " fail. error:" << errno << ','
             << strerror(errno) << endl;
        return false;
    }
    ...
    do {
        ...
        util::fs::IsFileExist(log_filename));  //! 避免在同一秒多次创建日志文件，都指向同一日志名
    log_filename_ = std::move(log_filename);

    int flags = O_CREAT | O_WRONLY | O_APPEND;
    if (file_sync_enable_) flags |= O_DSYNC;

    fd_ = ::open(log_filename_.c_str(), flags, S_IRUSR | S_IWUSR);
    if (fd_ < 0) {
        cerr << "Err: open file " << log_filename_ << " fail. error:" << errno << ','
             << strerror(errno) << endl;
        return false;
    }

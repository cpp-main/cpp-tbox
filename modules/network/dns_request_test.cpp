...
const std::vector<IPAddress> dns_srv_ip_vec = {IPAddress::FromString("114.114.114.114"),
...
TEST(DnsRequest, request_baidu)
{
    ...
    dns.request(DomainName("www.baidu.com"), [&](const DnsRequest::Result &result) {
...
TEST(DnsRequest, request_baidu_and_bing)
{
    ...
    bool bing_tag = false;
    dns.request(DomainName("cn.bing.com"), [&](const DnsRequest::Result &result) {
        EXPECT_EQ(result.status, DnsRequest::Result::Status::kSuccess);
        EXPECT_NE(result.a_vec.size(), 0);
        bing_tag = true;
#if 0
...
TEST(DnsRequest, timeout)
{
    ...
    const std::vector<IPAddress> invalid_dns_srv_ip_vec = {IPAddress::FromString("0.0.0.0")};

    bool tag = false;
    DnsRequest dns(sp_loop, invalid_dns_srv_ip_vec);
    dns.request(DomainName("www.baidu.com"), [&tag](const DnsRequest::Result &result) {

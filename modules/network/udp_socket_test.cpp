...
TEST(UdpSocket, echo)
{
    ...
    sp_timer->setCallback([&send_times, &udp_client, server_addr, sp_timer] {
        for (int i = 0; i < 100; ++i) udp_client.send("123456789", 10, server_addr);
        ++send_times;
        if (send_times == 100) sp_timer->disable();
    });
    ...
    sp_loop->runInLoop([sp_loop] { sp_loop->exitLoop(std::chrono::seconds(3)); });
...
TEST(UdpSocket, echo_connect)
{
    ...
    sp_timer->initialize(std::chrono::milliseconds(10), Event::Mode::kPersist);
    sp_timer->setCallback([&send_times, &udp_client, sp_timer] {
        for (int i = 0; i < 100; ++i) udp_client.send("123456789", 10);
        ++send_times;
        if (send_times == 100) sp_timer->disable();
    });

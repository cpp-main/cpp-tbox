...
TEST(Base64, Encode) {
    ...
    char out[37] = {0};
    EXPECT_EQ(Encode((const uint8_t*)(in), 26, out, 36), 36);
    EXPECT_STREQ(out, "QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVo=");
}

TEST(Base64, Encode_ReturnString) {
    const char* in = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    EXPECT_EQ(Encode((const uint8_t*)(in), 26), "QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVo=");
}

TEST(Base64, Encode_LenthNotEnough) {
    const char* in = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    char out[36] = {0};
    EXPECT_EQ(Encode((const uint8_t*)(in), 26, out, 35), 0);
}

TEST(Base64, Decode) {
    const char* in = "QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVo=";
    char out[27] = {0};
    EXPECT_EQ(Decode(in, 36, (uint8_t*)out, 26), 26);
...
TEST(Base64, Decode_LenthNotEnough) {
    const char* in = "QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVo=";
    char out[27] = {0};
    EXPECT_EQ(Decode(in, 36, (uint8_t*)out, 25), 0);
}

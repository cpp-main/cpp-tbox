...
int FindEndPos(const char *str_ptr, size_t str_len)
{
    ...
    for (size_t i = 0; i < str_len; ++i) {
        char ch = str_ptr[i];
        if (!is_started && ::isgraph(ch)) is_started = true;
        ...
        if (ch == '"') {
            if (in_string) {
                ...
                in_string = false;
                for (size_t j = (i - 1); j != 0 && str_ptr[j] == '\\'; --j) in_string = !in_string;
            } else {
                in_string = true;
            }
        } else {
        ...
        if (braces_level == 0 && square_level == 0 && !in_string && is_started) {
            return i + 1;
        }

        if (braces_level < 0 || square_level < 0) return -1;
    }

...
struct LoopInfo
{
    ...
    LoopInfo(event::Loop *l, const std::string &n) : loop(l), name(n), state(State::kChecked) {}
...
void LoopWDog::Register(event::Loop *loop, const std::string &name)
{
    ...
    auto iter = std::find_if(
        _loop_info_vec.begin(),
        _loop_info_vec.end(),
        [loop](const LoopInfo::SharedPtr &loop_info_sptr) { return loop_info_sptr->loop == loop; });

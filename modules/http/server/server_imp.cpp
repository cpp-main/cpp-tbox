...
Server::Impl::Impl(Server *wp_parent, Loop *wp_loop) : wp_parent_(wp_parent), tcp_server_(wp_loop)
{}
...
{
    if (!tcp_server_.initialize(bind_addr, listen_backlog)) return false;
...
void Server::Impl::cleanup()
{
    if (state_ != State::kNone) {
        ...
        for (auto conn : conns_) delete conn;
        conns_.clear();
...
void Server::Impl::onTcpReceived(const TcpServer::ConnToken &ct, Buffer &buff)
{
    ...
    while (buff.readableSize() > 0) {
        ...
        if (conn->req_parser.state() == RequestParser::State::kFinishedAll) {
            ...
            if (context_log_enable_) LogDbg("REQ: [%s]", req->toString().c_str());
            ...
            LogNotice("parse http from %s fail",
                      tcp_server_.getClientAddress(ct).toString().c_str());
...
void Server::Impl::commitRespond(const TcpServer::ConnToken &ct, int index, Respond *res)
{
    ...
    if (index == conn->res_index) {
        //! 将当前的数据直接发送出去
        {
            ...
            delete res;
            if (context_log_enable_) LogDbg("RES: [%s]", content.c_str());
        }
...
{
    if (cb_index >= req_cb_.size()) return;

    auto func = req_cb_.at(cb_index);

    ++cb_level_;
    if (func) func(sp_ctx, std::bind(&Impl::handle, this, sp_ctx, cb_index + 1));
    --cb_level_;
}
...
{
    //! 一定要记得清除非发送的Respond
    for (auto &item : res_buff) delete item.second;
}

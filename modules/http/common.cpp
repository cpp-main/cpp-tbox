...
std::string HttpVerToString(HttpVer ver)
{
    ...
    auto iter = std::find_if(
        table_begin, table_end, [=](const HttpVerPair &item) { return item.first == ver; });
    if (iter != table_end)
        return iter->second;
    else
        return "";
...
HttpVer StringToHttpVer(const std::string &str)
{
    ...
    auto table_end = _http_ver_map + NUMBER_OF_ARRAY(_http_ver_map);
    auto iter = std::find_if(
        table_begin, table_end, [=](const HttpVerPair &item) { return item.second == str; });
...
std::string MethodToString(Method ver)
{
    ...
    auto iter = std::find_if(
        table_begin, table_end, [=](const MethodPair &item) { return item.first == ver; });
    if (iter != table_end)
        return iter->second;
    else
        return "";
...
Method StringToMethod(const std::string &str)
{
    ...
    auto table_end = _method_map + NUMBER_OF_ARRAY(_method_map);
    auto iter = std::find_if(
        table_begin, table_end, [=](const MethodPair &item) { return item.second == str; });
...
std::string StatusCodeToString(StatusCode code)
{
    ...
    auto iter = std::find_if(
        table_begin, table_end, [=](const StatusCodePair &item) { return item.first == code; });
    if (iter != table_end)
        return iter->second;
    else
        return "";
...
StatusCode StringToStatusCode(const std::string &str)
{
    ...
    auto table_end = _status_code_map + NUMBER_OF_ARRAY(_status_code_map);
    auto iter = std::find_if(
        table_begin, table_end, [=](const StatusCodePair &item) { return item.second == str; });
